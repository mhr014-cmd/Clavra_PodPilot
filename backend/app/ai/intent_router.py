...
def _extract_order_ref(message: str, entities: dict) -> str | None:
    ...
    m = re.search(r'\b(PO-?\d+|ORD-?\d+|P\d{2,})\b', message, re.IGNORECASE)
    if m:
        candidate = m.group()
    ...
    try:
        if intent.intent == "get_active_orders":
            ...
            response_text = (
                ...
                "**🚢 Shipments** — *'List shipments'*, *'Track SHP-001'*, *'Cancel SHP-002'*\n"
                "**📦 Inventory** — *'Check my stock'*, *'How much cotton do I have?'*\n"
                "**📊 Analytics** — *'How many units this week?'*, *'Line efficiency report'*\n"
                "**📄 Documents** — *'Quality inspection policy'*, *'SOP for cutting'*\n"
            ...
            if not order_id and shipment_hint:
                ...
                result = await get_last_orders(db, org_id, 5)
                data = result
                if result:
            ...
            if not shipment_no:
                result = await get_recent_shipments(db, org_id, 5)
                data = result
                if result:
