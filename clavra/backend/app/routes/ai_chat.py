...
async def confirm_action(
    ...
    if action_type == "cancel_order":
        ...
        from app.services.production_service import create_order_via_ai
        result = await create_order_via_ai(
            db, order_no=data["order_no"], buyer=data["buyer"],
            style=data["style"], quantity=int(data["quantity"]),
            org_id=org_id, user_id=user_id,
        )
        return {"success": result.get("success", False), "message": result.get("message")}

    elif action_type == "book_shipment":
        import json as _json
        data = _json.loads(ref)
        from app.services.shipment_service import create_shipment_via_ai
        result = await create_shipment_via_ai(
            db, shipment_no=data["shipment_no"], org_id=org_id,
            destination=data.get("destination"), carrier=data.get("carrier"),
        ...
        import json as _json
        data = _json.loads(ref)
        from app.services.production_service import update_order_status_ai
        result = await update_order_status_ai(
            db, order_ref=data["order_no"], new_status=data["new_status"], org_id=org_id,
        )
        return {"success": result.get("success", False), "message": result.get("message")}

    elif action_type == "log_quality":
        import json as _json
        data = _json.loads(ref)
        from app.services.production_service import log_defects_ai
        result = await log_defects_ai(
            db, order_ref=data["order_no"], defect_qty=int(data["defect_qty"]),
            org_id=org_id, defect_type=data.get("defect_type", "General"),
        )
        return {"success": result.get("success", False), "message": result.get("message")}
